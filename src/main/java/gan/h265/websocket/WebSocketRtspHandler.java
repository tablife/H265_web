...
public class WebSocketRtspHandler extends AbstractWebSocketHandler {
    ...
    final static String Tag = WebSocketRtspHandler.class.getName();
    ...
    public void afterConnectionEstablished(WebSocketSession session) throws Exception {
        WebSocketServerManager.fileLog(Tag,"onOpen:"+session.getId());
    ...
    public void handleTransportError(WebSocketSession session, Throwable exception) throws Exception {
        WebSocketServerManager.fileLog(Tag,"onError:"+session.getId());
        WebSocketHandler handler = findWebSocketHandler(session);
        if(handler!=null){
    ...
    public void afterConnectionClosed(WebSocketSession session, CloseStatus closeStatus) throws Exception {
        WebSocketServerManager.fileLog(Tag,"onClose:"+session.getId());
        WebSocketHandler handler = findWebSocketHandler(session);
        if(handler!=null){
    ...
    protected void handleTextMessage(WebSocketSession session, TextMessage message) throws Exception {
        ...
        DebugLog.d(Tag,"onTextMessage:"+session.getId());
        WebSocketHandler handler = findWebSocketHandler(session);
        if(handler!=null){
    ...
    protected void handleBinaryMessage(WebSocketSession session, BinaryMessage message) throws Exception {
        ...
        DebugLog.d(Tag,"onBinaryMessage:"+session.getId());
        WebSocketHandler handler = findWebSocketHandler(session);
        if(handler!=null){
            handler.handleMessage(session,message);
        }else{
